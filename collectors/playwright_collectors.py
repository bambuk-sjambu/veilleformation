    ...
    def collect(self) -> list[dict]:
        ...
        return articles
            ...
            try:
                ...
                # Take screenshot for                # page.screenshot(path="data/screenshots/travail_gouv_debug.png")
...
class EducationGouvPlaywrightCollector(PlaywrightBaseCollector):
    """Playwright scraper for Education.gouv actualites."""

    SOURCE_NAME = "education_gouv"
    START_URL = "https://www.education.gouv.fr/actualites"
    ...
    def collect(self) -> list[dict]:
        ...
        with sync_playwright() as p:
            ...
            try:
                ...
                # Extract news items
                elements = page.query_selector_all("article, .news-item, .post, .card, .teaser")
                ...
                for el in elements:
                    ...
                    title_el = el.query_selector("h2, h3, h4, .title")
                    title = title_el.inner_text() if title_el else link.inner_text()
                    ...
                    if any(kw in text_lower for kw in ["reglement", "decret", "arrete", "circulaire"]):
                        category = "reglementaire"
                    elif any(kw in text_lower for kw in ["financement", "aide", "bourse"]):
                ...
                self.logger.info(f"Education.gouv PW: {len(articles)} items")

            except Exception as e:
                self.logger.error(f"Education.gouv PW error: {e}")
            finally:
                self.close()
